...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const normalizedPassword =
      normalizePhoneNumber(password);

    if (!normalizedUsername || !normalizedPassword) {
      return res.status(400).json({
        message:
          'Username dan password nomor telepon wajib diisi.'
      });
    }
    ...
    const lookupCandidates =
      getPhoneCandidates(normalizedUsername);
...
app.get('/', (req, res) => {
  res.send(
    'Server Absensi Backend berjalan dengan baik!'
  );
});
